...
plt.axis( [0, len( combined_bin_min_errors ), 0, max( combined_bin_model_errors + combined_bin_no_model_errors )] )
...
plt.legend( handles = [line1, line2, line3, line4], loc = 'lower left' )
...
bin_labels = pd.Series( [helpers.sale_price_to_bin( boundaries, sp ) for sp in new_train[predict_column]] )
# partition_bin = helpers.create_partitions_for_k_fold_validation( ideal_bins, folds, boundaries, bin_data, predict_column )
# dt = RandomForestClassifier( n_estimators = 20 )
dt = dt.fit( new_train[attributes_dt], bin_labels )
